...
1. [Install Decap via a package manager](https://decapcms.org/docs/install-decap-cms/#installing-with-npm) with the following command:
   ...
   <PackageManagerTabs>
     <Fragment slot="npm">```shell npm install decap-cms-app ```</Fragment>
     <Fragment slot="pnpm">```shell pnpm add decap-cms-app ```</Fragment>
     <Fragment slot="yarn">```shell yarn add decap-cms-app ```</Fragment>
   </PackageManagerTabs>
...
2. Add `config.yml` to `public/admin/`:
   ...
   <FileTree>- public - admin - config.yml</FileTree>
...
4. Add the `admin` route for your React app in `src/pages/admin.html`.
   ...
   <FileTree>- public - admin - config.yml - src - pages - admin.html</FileTree>
