...
<ReadMore>
  Read more about [environment variables](/en/guides/environment-variables/) and
  `.env` files in Astro.
</ReadMore>
...
<PackageManagerTabs>
  <Fragment slot="npm">```shell npm install @scalekit-sdk/node ```</Fragment>
  <Fragment slot="pnpm">```shell pnpm add @scalekit-sdk/node ```</Fragment>
  <Fragment slot="yarn">```shell yarn add @scalekit-sdk/node ```</Fragment>
</PackageManagerTabs>
...
<FileTree title="Project Structure">
  - src/ - lib/ - **scalekit.ts** - env.d.ts - .env - package.json
</FileTree>
...
<FileTree title="Project Structure">
  - src/ - lib/ - scalekit.ts - pages/ - api/ - auth/ - **login.ts** -
  **callback.ts** - **logout.ts** - env.d.ts - .env - astro.config.mjs -
  package.json
</FileTree>
