...
```astro
<!-- This -->
<span class:list={["hello goodbye", { world: true }, ["friend"]]}></span>
<!-- Becomes -->
...
<article set:html={api.getArticle(Astro.props.id)} />
```
...
```astro
<article set:html={fetch("http://example/old-posts/making-soup.html")} />
```
...
<ReadMore>
  See the [Styling & CSS](/en/guides/styling/#global-styles) page for more
  details about how global styles work.
</ReadMore>
...
<ReadMore>
  See how [client-side scripts](/en/guides/client-side-scripts/) work in Astro
  components.
</ReadMore>
